...
function UF:Update_Raid35Header(header, db)
	...
	if not parent.positioned then
		...
		E:CreateMover(parent, parent:GetName()..'Mover', L["Raid-35 Frames"], nil, nil, nil, 'ALL,RAID', nil, 'unitframe,groupUnits,raid35,generalGroup')
		parent.positioned = true
	end
end

function UF:Update_Raid35Frames(frame, db)
	frame.db = db
